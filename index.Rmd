...
`roxygen2` `r citep(bib[['roxygen2']])` you will need to document your functions in the `.R` files with comments formatted in a specific way. Right
...
When you have saved functions with `roxygen2` `r citep(bib[['roxygen2']])` style comments you can create the `.Rd` files (the man files themselves) 
by running:
...
Please read [Hadley](http://had.co.nz/)'s [guide](http://adv-r.had.co.nz/Documenting-functions.html) in its entirety to understand how to document packages and in particular, how `roxygen2` `r citep(bib[['roxygen2']])` 
...
* [Leo Collado Torres](http://www.biostat.jhsph.edu/~lcollado/#.UlP1O2TXis0) fixed some typos/added links, used `knitrBootstrap` `r citep(bib[['knitrBootstrap']])` to format the page.
